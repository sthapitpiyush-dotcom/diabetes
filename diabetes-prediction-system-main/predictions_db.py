...
def load_predictions():
    """Load all predictions from file with caching"""
...
def save_predictions(predictions):
    """Save predictions to file and clear cache"""
    with open(PREDICTIONS_FILE, 'w') as f:
        json.dump(predictions, f, indent=2)
    # Clear the cache after saving
    st.cache_data.clear()

def add_prediction(username, features, prediction, probability, recommended_doctor=None):

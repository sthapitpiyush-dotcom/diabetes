...
class DiabetesPredictor:
    ...
    def transform_features(self):
        ...
        target_name = 'Outcome'
        x_features = self.df_selected.drop(target_name, axis=1)
        y_target = self.df_selected[target_name]
        
        # Fit quantile transformer only on features (not Outcome)
        X_transformed = self.quantile.fit_transform(x_features)
        self.df_new = pd.DataFrame(X_transformed)
        self.df_new.columns = ['Pregnancies', 'Glucose', 'SkinThickness', 'BMI', 'Age']
        # Add Outcome back
        self.df_new[target_name] = y_target.values
        return self.df_new
    ...
    def train_knn_tuned_only(self):
        """Train only KNN Tuned model for faster loading"""
        print("Training KNN Tuned (Best Model)...")
        self.tune_knn()
        print("Training complete!")
    
    def save_model(self, filepath="saved_models/diabetes_model.pkl"):
        """Save the trained model and preprocessor to disk for faster loading"""
        os.makedirs(os.path.dirname(filepath), exist_ok=True)
        model_data = {
            'model': self.models.get('knn_tuned', self.best_model),
            'quantile_transformer': self.quantile,
            'feature_names': ['Pregnancies', 'Glucose', 'SkinThickness', 'BMI', 'Age']
        }
        joblib.dump(model_data, filepath, compress=3)
        print(f"Model saved to {filepath}")
    
    def load_saved_model(self, filepath="saved_models/diabetes_model.pkl"):
        """Load a previously saved model from disk"""
        if not os.path.exists(filepath):
            return False
        try:
            model_data = joblib.load(filepath)
            self.models['knn_tuned'] = model_data['model']
            self.best_model = model_data['model']
            self.quantile = model_data['quantile_transformer']
    ...
    def save_confusion_matrix_plot(self, cm, filepath="saved_models/confusion_matrix.png"):
        """Save confusion matrix as a heatmap visualization"""
        os.makedirs(os.path.dirname(filepath), exist_ok=True)
        ...
        plt.title('Confusion Matrix - KNN Tuned Model')
        plt.tight_layout()
        plt.savefig(filepath, dpi=100, bbox_inches='tight')
        plt.close()
        print(f"Confusion matrix plot saved to {filepath}")
    
    def save_metrics_json(self, metrics, filepath="saved_models/model_metrics.json"):
...
if __name__ == "__main__":
    ...
    for model_name, model in predictor.models.items():
        ...
        # Save best model metrics and visualization
        if model_name == 'knn_tuned':
            best_metrics = metrics
            predictor.save_confusion_matrix_plot(metrics['confusion_matrix'])
            predictor.save_metrics_json(metrics)
